# MyST Writer Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a `docmux-writer-myst` crate that serializes the docmux AST to MyST Markdown, completing the last format gap for pandoc parity.

**Architecture:** A standalone crate implementing the `Writer` trait. Emits standard CommonMark for basic nodes, MyST directive syntax (`:::{name}`) for admonitions/figures/code-blocks, MyST roles (`` {role}`text` ``) for citations/cross-refs/sub/sup/underline, and `(label)=` target syntax for labelled elements. Reuses the default markdown template for standalone mode.

**Tech Stack:** Rust, `docmux-ast`, `docmux-core`, `docmux-template`

**Spec:** `docs/superpowers/specs/2026-04-10-myst-writer-design.md`

---

### Task 1: Scaffold the crate and write the Writer trait impl with a basic paragraph test

**Files:**
- Create: `crates/docmux-writer-myst/Cargo.toml`
- Create: `crates/docmux-writer-myst/src/lib.rs`
- Modify: `Cargo.toml` (workspace root — add to `members` and `workspace.dependencies`)

- [ ] **Step 1: Create `crates/docmux-writer-myst/Cargo.toml`**

```toml
[package]
name = "docmux-writer-myst"
version.workspace = true
edition.workspace = true
license.workspace = true
repository.workspace = true
description = "MyST Markdown writer for docmux"
...
- [ ] **Step 2: Create minimal `crates/docmux-writer-myst/src/lib.rs`**

Start with just the struct, the `Writer` trait impl, and the `write_blocks`/`write_block` skeleton that handles only `Paragraph`:
...
- [ ] **Step 3: Add the crate to the workspace root `Cargo.toml`**

In `Cargo.toml` at the workspace root:
- Add `"crates/docmux-writer-myst"` to `[workspace] members` (after `docmux-writer-markdown`)
- Add `docmux-writer-myst = { path = "crates/docmux-writer-myst" }` to `[workspace.dependencies]` (after `docmux-writer-markdown`)

- [ ] **Step 4: Write the failing test for paragraph rendering**

Add to the bottom of `crates/docmux-writer-myst/src/lib.rs`:
...
Run: `cargo test -p docmux-writer-myst`
Expected: 2 tests PASS

- [ ] **Step 6: Commit**

```bash
git add crates/docmux-writer-myst/ Cargo.toml
git commit -m "feat(myst-writer): scaffold crate with paragraph rendering"
```

---

### Task 2: Basic block nodes — heading, code block, math block, blockquote, thematic break

**Files:**
- Modify: `crates/docmux-writer-myst/src/lib.rs`

- [ ] **Step 1: Write failing tests**

Add to the `tests` module in `crates/docmux-writer-myst/src/lib.rs`:
...
Run: `cargo test -p docmux-writer-myst`
Expected: 8 new tests FAIL (heading_no_id, heading_with_id, code_block_simple, code_block_with_caption, math_block_no_label, math_block_with_label, blockquote, thematic_break)

- [ ] **Step 3: Implement the block handlers**

In `write_block`, replace the `_ => {}` catch-all with these match arms:
...
```rust
            ...
            _ => {} // remaining blocks in next tasks
```

Add two helper methods to `impl MystWriter`:

```rust
    fn write_code_block_fenced(
        &self,
...
```bash
git add crates/docmux-writer-myst/src/lib.rs
git commit -m "feat(myst-writer): add heading, code block, math block, blockquote, thematic break"
```

---

### Task 3: Lists, tables, definition lists, raw blocks
    ...
    fn ordered_list() {
        let doc = Document {
            content: vec![Block::List {
                ...
                start: Some(3),
                items: vec![
                    ListItem { checked: None, content: vec![Block::text("Third")] },
                    ListItem { checked: None, content: vec![Block::text("Fourth")] },
                ],
                tight: true,
                style: None,
                delimiter: None,
    ...
    fn task_list() {
        let doc = Document {
            content: vec![Block::List {
                ...
                start: None,
                items: vec![
                    ListItem { checked: Some(true), content: vec![Block::text("Done")] },
                    ListItem { checked: Some(false), content: vec![Block::text("Todo")] },
                ],
                tight: true,
                style: None,
                delimiter: None,
...
- [ ] **Step 3: Implement list, table, definition list, raw block handlers**

Add these match arms to `write_block` (before the `_ => {}`):
...
Add these helper methods to `impl MystWriter`:
...
```bash
git add crates/docmux-writer-myst/src/lib.rs
git commit -m "feat(myst-writer): add list, table, definition list, raw block"
```

---

### Task 4: MyST-specific blocks — admonitions, figures, footnote defs, divs
...
- [ ] **Step 3: Implement admonition, figure, footnote def, div handlers**
...
```rust
            ...
            Block::Div { attrs, content } => {
                out.push_str(":::");
                let has_attrs = attrs.id.is_some()
...
Now remove the `_ => {}` catch-all — all block variants should be covered.
...
- [ ] **Step 5: Run clippy to check for exhaustive match**

Run: `cargo clippy -p docmux-writer-myst -- -D warnings`
Expected: No warnings (all Block variants handled)

- [ ] **Step 6: Commit**

```bash
git add crates/docmux-writer-myst/src/lib.rs
git commit -m "feat(myst-writer): add admonitions, figures, footnotes, divs"
```

---

### Task 5: All inline nodes — standard Markdown inlines
    ...
    fn emphasis_and_strong() {
        let doc = Document {
            content: vec![Block::Paragraph {
                content: vec![
                    Inline::Emphasis { content: vec![Inline::text("italic")] },
                    Inline::text(" and "),
                    Inline::Strong { content: vec![Inline::text("bold")] },
                ],
            }],
            ..Default::default()
        };
    ...
    fn inline_code() {
        let doc = Document {
            content: vec![Block::Paragraph {
                content: vec![
                    Inline::text("Use "),
                    Inline::Code { value: "fn main()".into(), attrs: None },
                ],
            }],
            ..Default::default()
        };
    ...
    fn inline_math() {
        let doc = Document {
            content: vec![Block::Paragraph {
                content: vec![
                    Inline::text("Energy: "),
                    Inline::MathInline { value: "E = mc^2".into() },
                ],
            }],
            ..Default::default()
        };
...
- [ ] **Step 3: Implement all standard inline handlers**

Replace the `write_inline` method with the full implementation:
...
```rust
    fn write_inline(&self, inline: &Inline, out: &mut String) {
        match inline {
            ...
            // MyST-specific inlines handled in next task
            Inline::Citation(_)
            | Inline::CrossRef(_)
            | Inline::Superscript { .. }
            | Inline::Subscript { .. }
...
Add this helper function outside the impl block:
...
```bash
git add crates/docmux-writer-myst/src/lib.rs
git commit -m "feat(myst-writer): add all standard inline nodes"
```

---

### Task 6: MyST-specific inlines — citations, cross-refs, sub/sup/underline roles
...
```rust
        ...
        let doc = Document {
            content: vec![Block::Paragraph {
                content: vec![Inline::Citation(Citation {
                    items: vec![
                        CiteItem { key: "smith2020".into(), prefix: None, suffix: None },
                        CiteItem { key: "jones2021".into(), prefix: None, suffix: None },
                    ],
    ...
    fn citation_author_only() {
        let doc = Document {
            content: vec![Block::Paragraph {
                content: vec![Inline::Citation(Citation {
                    items: vec![CiteItem { key: "smith2020".into(), prefix: None, suffix: None }],
    ...
    fn superscript_role() {
        let doc = Document {
            content: vec![Block::Paragraph {
                content: vec![
                    Inline::text("x"),
                    Inline::Superscript { content: vec![Inline::text("2")] },
                ],
            }],
            ..Default::default()
        };
    ...
    fn subscript_role() {
        let doc = Document {
            content: vec![Block::Paragraph {
                content: vec![
                    Inline::text("H"),
                    Inline::Subscript { content: vec![Inline::text("2")] },
                    Inline::text("O"),
                ],
            }],
            ..Default::default()
...
- [ ] **Step 3: Implement MyST-specific inline handlers**

In `write_inline`, replace the placeholder arms at the bottom with:
...
- [ ] **Step 5: Run clippy to verify exhaustive match**

Run: `cargo clippy -p docmux-writer-myst -- -D warnings`
Expected: No warnings (all Inline variants handled)

- [ ] **Step 6: Commit**

```bash
git add crates/docmux-writer-myst/src/lib.rs
git commit -m "feat(myst-writer): add citations, cross-refs, sub/sup/underline roles"
```

---

### Task 7: Standalone mode (frontmatter) and template support

**Files:**
- Modify: `crates/docmux-writer-myst/src/lib.rs`
- Modify: `crates/docmux-template/src/lib.rs`
- Create: `crates/docmux-template/templates/default.myst` (copy of `default.markdown`)
...
Run: `cargo test -p docmux-writer-myst standalone_with_frontmatter`
Expected: FAIL

- [ ] **Step 3: Create `crates/docmux-template/templates/default.myst`**

Copy the content of `default.markdown` (the template is identical — MyST uses YAML frontmatter just like Markdown):
...
- [ ] **Step 4: Register the template in `crates/docmux-template/src/lib.rs`**

Add after the `DEFAULT_MARKDOWN` constant:
...
Add to the `default_template_for` match:

```rust
        "myst" => Some(DEFAULT_MYST),
```

Add to the `default_templates_parse_without_error` test array:

```rust
            ("myst", DEFAULT_MYST),
```

- [ ] **Step 5: Implement standalone mode in the writer**

Add these methods to `impl MystWriter` in `crates/docmux-writer-myst/src/lib.rs`:
...
Add these helper functions outside the impl:
...
Update the `write` method in the `Writer` impl to support standalone:
...
Run: `cargo test -p docmux-writer-myst && cargo test -p docmux-template`
Expected: All tests PASS in both crates

- [ ] **Step 7: Commit**

```bash
git add crates/docmux-writer-myst/src/lib.rs crates/docmux-template/
git commit -m "feat(myst-writer): add standalone mode with YAML frontmatter"
```

---

### Task 8: Register in CLI, WASM, and playground
...
- [ ] **Step 1: Add dependency to `crates/docmux-cli/Cargo.toml`**

Add after `docmux-writer-markdown`:

```toml
docmux-writer-myst = { workspace = true }
```

- [ ] **Step 2: Register in `crates/docmux-cli/src/main.rs`**

Add import at the top (after `use docmux_writer_markdown::MarkdownWriter;`):

```rust
use docmux_writer_myst::MystWriter;
```

Add registration in the `build_registry` function (after the `MarkdownWriter` line):

```rust
    reg.add_writer(Box::new(MystWriter::new()));
```

- [ ] **Step 3: Add dependency to `crates/docmux-wasm/Cargo.toml`**

In the `# Writers` section, add after `docmux-writer-markdown`:

```toml
docmux-writer-myst = { workspace = true }
```

- [ ] **Step 4: Register in `crates/docmux-wasm/src/lib.rs`**

Add import (after `use docmux_writer_markdown::MarkdownWriter;`):

```rust
use docmux_writer_myst::MystWriter;
```

Add registration in `build_registry` (after `MarkdownWriter` line):

```rust
    reg.add_writer(Box::new(MystWriter::new()));
```

- [ ] **Step 5: Verify full workspace builds and tests pass**
...
```bash
git add crates/docmux-cli/Cargo.toml crates/docmux-cli/src/main.rs crates/docmux-wasm/Cargo.toml crates/docmux-wasm/src/lib.rs
git commit -m "feat(myst-writer): register in CLI and WASM"
```
